...
Spec.prototype.completionCheck = function (force) {
  ...
  if (!spec.testsPending || force) {
    ...
    /**
     * Emit any pending tests as errors
     */
    if (spec.testsPending) {
...
Spec.prototype.githubMarkdown = function () {
  ...
  for (i = 0; i < spec.nodes.length; i++) {
    ...
    var toc = textToAnchor(spec.tocDesc) || 'table-of-contents';
    if (!node.muted) {
      if (i)
        text += '\n';
      switch (node.type) {
        case 't':
          if (i === 0) {
            ...
            text += '## [:arrow_up:](' + getPreviousLink(i) + ')&nbsp;[:top:](#' + toc + ') &nbsp;' + node.text;
          } else {
            text += '## [:arrow_up:](' + getPreviousLink(i) + ')&nbsp;[:top:](#' + toc + ')&nbsp;[:arrow_down:](' + getNextLink(i) + ') &nbsp;' + node.text;
          }
          break;
        case 'h':
          text += '#### ' + node.text;
          break;
        case 'i':
          text += generateTOC(spec.tocDesc || 'Table of Contents');
          break;
        case 'e':
          text += codeBlock();
          break;
  ...
  function textToAnchor(text) {
    var headless = text.replace(/#/g, '');
    return headless.toLowerCase().replace(/ /g, '-');
  }
